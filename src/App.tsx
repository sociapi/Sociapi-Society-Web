  ...
  return (
    <header className="sticky top-0 z-40 px-4 pt-4">
      <nav className={`mx-auto flex max-w-7xl items-center justify-between rounded-2xl border px-4 py-3 shadow-xl shadow-black/20 backdrop-blur-2xl transition-all duration-300 sm:px-6 sm:py-3.5 ${scrolled ? "border-[#e8ecee]/[0.14] bg-[#333333]/70" : "border-[#e8ecee]/[0.11] bg-[#333333]/45"}`}>
        <a href="#home" className="flex shrink-0 items-center gap-2.5 transition hover:opacity-90"><Logo className="h-9 w-9 text-lg" /><span className="hidden font-heading text-base font-bold tracking-[0.28em] text-[#e8ecee] sm:block">SOCIAPI</span></a>
        <div className="hidden items-center gap-0.5 lg:flex">
          {pages.slice(0, 8).map((page) => <a key={page.key} className={linkCls(page.key)} href={`#${page.key}`}>{page.label}</a>)}
          <div className="relative" onMouseEnter={() => setMoreOpen(true)} onMouseLeave={() => setMoreOpen(false)}>
            ...
            <div className={`absolute right-0 top-full mt-2 w-52 origin-top-right rounded-2xl border border-[#e8ecee]/12 bg-[#333333]/96 p-2 shadow-2xl backdrop-blur-xl transition-all duration-200 ${moreOpen ? "visible scale-100 opacity-100" : "invisible scale-95 opacity-0 pointer-events-none"}`}>
              {pages.slice(8).map((page) => <a key={page.key} href={`#${page.key}`} className={`block rounded-xl px-4 py-2.5 text-sm font-medium transition ${route === page.key ? "bg-[#7bd355]/15 text-[#7bd355]" : "text-[#e8ecee]/70 hover:bg-[#e8ecee]/[.08] hover:text-[#e8ecee]"}`}>{page.label}</a>)}
            </div>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <a href="#contact" className="magnetic hidden rounded-full bg-[#7bd355] px-6 py-2.5 text-sm font-bold text-[#0c140a] lg:inline-block">Join Community</a>
      ...
      {open && (
        <>
          ...
          <div className="mx-auto mt-3 grid max-w-7xl animate-slide-up gap-1 overflow-hidden rounded-2xl border border-[#e8ecee]/10 bg-[#333333]/95 p-3 backdrop-blur-xl lg:hidden">
            {pages.map((page) => <a key={page.key} href={`#${page.key}`} onClick={() => setOpen(false)} className={`flex items-center justify-between rounded-xl px-4 py-3.5 transition ${route === page.key ? "bg-[#7bd355]/15 text-[#7bd355] font-bold" : "text-[#e8ecee]/75 hover:bg-[#e8ecee]/[.08] hover:text-[#e8ecee]"}`}>{page.label}<span className="text-[10px] opacity-50">→</span></a>)}
            <a href="#contact" onClick={() => setOpen(false)} className="mt-2 block rounded-2xl bg-[#7bd355] py-3.5 text-center text-sm font-bold text-[#0c140a]">Join Community</a>
          </div>
        </>
      )}
    </header>
  ...
  return (
    <>
      <section className="relative -mt-[88px] flex min-h-screen flex-col overflow-hidden bg-[#050705] text-[#e8ecee]" onMouseMove={onMove}>
        ...
        {/* Layer 2 — orbit rings (parallax with mouse) */}
        {!isMobile && (
        ...
        <div className="relative z-20 mx-auto flex w-full max-w-7xl flex-1 flex-col items-center justify-center px-5 pb-10 pt-32 text-center sm:pt-36">
          ...
          <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 2.05 }} className="mt-10 flex flex-wrap items-center justify-center gap-4">
            <a href="#contact" className="cta-premium text-base">
...
/* ---------------- Team Galaxy (original orbiting circles) ---------------- */
...
/* ---------------- Community Partners (replaces Ecosystem) ---------------- */
  ...
  return (
    <section className="section">
      ...
      <Reveal className="mx-auto mt-12 max-w-6xl rounded-[2rem] border border-[#7bd355]/20 bg-[#7bd355]/10 p-6 text-center sm:rounded-[2.5rem] sm:p-8">
        ...
        <a href="#contact" className="magnetic mt-6 inline-flex rounded-full bg-[#7bd355] px-8 py-4 text-base font-bold text-[#0c140a]">Start Membership</a>
      </Reveal>
    </section>
  );
}
...
function Product({ name, image, description, price, onPreview, onAdd }: { name: string; image: string; description: string; price: string; onPreview: (s: string) => void; onAdd: (item: { name: string; size: string; qty: number }) => void }) {
  ...
  return (
    <div className="rounded-[1.75rem] border border-[#e8ecee]/10 bg-[#e8ecee]/[.06] p-4 backdrop-blur-xl sm:rounded-[2rem] sm:p-5">
      <button onClick={() => onPreview(image)} className="w-full"><Avatar src={image} name={name} className="aspect-square w-full rounded-[1.25rem] object-cover sm:rounded-[1.5rem]" /></button>
...
function StickyJoin({ route }: { route: PageKey }) {
  ...
  return (
    <div className={`fixed inset-x-0 bottom-0 z-40 flex justify-center px-4 pb-4 transition-all duration-300 lg:hidden ${scrolled ? "translate-y-0 opacity-100" : "pointer-events-none translate-y-6 opacity-0"}`}>
      <a href="#contact" className="pointer-events-auto flex items-center gap-2 rounded-full bg-[#7bd355] px-7 py-3.5 text-base font-bold text-[#0c140a] shadow-2xl shadow-[#7bd355]/40 ring-1 ring-black/10">Join Community →</a>
    </div>
  );
}
    ...
    <footer className="relative border-t border-[#e8ecee]/10 px-5 pt-14 pb-28 text-[#e8ecee] sm:pb-14">
      <Reveal className="mx-auto mb-14 max-w-7xl overflow-hidden rounded-[2rem] border border-[#7bd355]/25 bg-gradient-to-r from-[#7bd355]/15 via-[#517642]/10 to-transparent p-7 sm:rounded-[2.5rem] sm:p-10">
        <div className="flex flex-col items-center gap-5 text-center md:flex-row md:justify-between md:text-left">
          ...
          <a href="#contact" className="magnetic shrink-0 rounded-full bg-[#7bd355] px-8 py-4 text-base font-bold text-[#0c140a]">Join Community →</a>
        </div>
      </Reveal>
      <div className="mx-auto grid max-w-7xl gap-10 md:grid-cols-[1.4fr_.8fr_.8fr_.9fr]">
        <div>
          ...
          <div className="mt-5 flex flex-wrap items-center gap-3">
            {socials.map((s) => <a key={s.label} href={s.href} target="_blank" rel="noreferrer" aria-label={s.label} className="flex h-11 w-11 items-center justify-center rounded-xl border border-[#e8ecee]/12 bg-[#e8ecee]/6 text-[#e8ecee]/75 transition hover:-translate-y-0.5 hover:border-[#7bd355]/40 hover:bg-[#7bd355] hover:text-[#0c140a]">{s.icon}</a>)}
          </div>
        </div>
        <div>
...
function SeoSchema() {
  const schema = { "@context": "https://schema.org", "@type": "Organization", name: "Sociapi Society", url: "https://linktr.ee/sociapisociety", email: "sociapisociety@gmail.com", telephone: "+92 332 9984490", address: "Islamia College University Peshawar, Pakistan" };
